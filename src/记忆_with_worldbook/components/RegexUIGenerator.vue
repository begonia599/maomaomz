...
const loadExample = () => {
  aiPrompt.value =
    '现代扁平风格的角色状态栏，包含3个标签页：\n1. 基础信息（姓名、年龄、性别、职业）\n2. 状态属性（生命值、魔法值、体力值、精力值）\n3. 关系面板（好感度、信任度、关系状态）\n\n设计要求：\n- **严格禁止使用任何渐变色（linear-gradient）**，只使用纯色\n- 使用扁平化设计，简洁的配色方案（如：深灰、白色、蓝色点缀）\n- 标签页使用纯色背景，活跃状态用不同的纯色区分\n- 字段行使用简洁的边框或背景色，不要花哨的装饰\n- 最小化阴影效果，只在必要时使用淡淡的阴影\n- 不使用任何 emoji，使用纯文字\n- 整体风格要扁平、简洁、克制，避免过度设计';
